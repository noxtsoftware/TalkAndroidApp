...
import com.moyn.talk.activities.FullScreenImageActivity;
import com.moyn.talk.activities.FullScreenMediaActivity;
import com.moyn.talk.activities.FullScreenTextViewerActivity;
import com.moyn.talk.application.NextcloudTalkApplication;
import com.moyn.talk.components.filebrowser.models.BrowserFile;
import com.moyn.talk.components.filebrowser.models.DavResponse;
import com.moyn.talk.components.filebrowser.webdav.ReadFilesystemOperation;
...
import com.moyn.talk.utils.AccountUtils;
import com.moyn.talk.utils.DisplayUtils;
import com.moyn.talk.utils.DrawableUtils;
...
public abstract class MagicPreviewMessageViewHolder extends MessageHolders.IncomingImageMessageViewHolder<ChatMessage> {
    ...
    public void onBind(ChatMessage message) {
        ...
        if (message.getMessageType() == ChatMessage.MessageType.SINGLE_NC_ATTACHMENT_MESSAGE) {
            ...
            String accountString = message.activeUser.getUsername() + "@" +
    ...
    private void updateViewsByProgress(String fileName, String mimetype, WorkInfo workInfo) {
        switch (workInfo.getState()) {
            case RUNNING:
                ...
                if (progress > -1) {
                    getMessageText().setText(String.format(context.getResources().getString(R.string.filename_progress),
                            fileName, progress));
                }
                break;
                        ...
                        if (davResponse.data != null) {
                            ...
                            if (!browserFileList.isEmpty()) {
                                new Handler(context.getMainLooper()).post(() -> {
                                    int resourceId = DrawableUtils.INSTANCE
