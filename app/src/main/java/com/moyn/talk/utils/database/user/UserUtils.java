 ...
 */
package com.moyn.talk.utils.database.user;
...
public class UserUtils {
    ...
    public List getUsers() {
        Result findUsersQueryResult = dataStore.select(User.class)
                .where(UserEntity.SCHEDULED_FOR_DELETION.notEqual(Boolean.TRUE)).get();
    ...
    public Observable<UserEntity> createOrUpdateUser(@Nullable String username, @Nullable String token,
        ...
        if (internalId == null) {
            findUserQueryResult = dataStore.select(User.class)
                    .where(UserEntity.USERNAME.eq(username).and(UserEntity.BASE_URL.eq(serverUrl))).limit(1).get();
        } else {
        ...
        if (user == null) {
            ...
            if ((displayName != null && user.getDisplayName() == null) || (displayName != null
                    && user.getDisplayName() != null && !displayName.equals(user.getDisplayName()))) {
                user.setDisplayName(displayName);
            }
