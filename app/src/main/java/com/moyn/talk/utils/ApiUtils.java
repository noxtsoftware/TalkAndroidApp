...
public class ApiUtils {
     ...
     */
    @Deprecated
    public static String getUrlForRemovingParticipantFromConversation(String baseUrl, String roomToken,
            boolean isGuest) {
    ...
    public static int getSignalingApiVersion(UserEntity user, int[] versions) throws NoSupportedApiException {
        for (int version : versions) {
            ...
            if (version == APIv1 &&
                    !CapabilitiesUtil.hasSpreedFeatureCapability(user, "signaling-v3")) {
                // Has no capability, we just assume it is always there when there is no v3 or
                // later
                return version;
            }
        }
    ...
    public static int getChatApiVersion(UserEntity user, int[] versions) throws NoSupportedApiException {
        for (int version : versions) {
            if (version == APIv1 && CapabilitiesUtil.hasSpreedFeatureCapability(user, "chat-v2")) {
                // Do not question that chat-v2 capability shows the availability of api/v1/
                // endpoint *see no evil*
                return version;
            }
        }
        throw new NoSupportedApiException();
    ...
    public static RetrofitBucket getRetrofitBucketForAddParticipant(int version, String baseUrl, String token,
            String user) {
        RetrofitBucket retrofitBucket = new RetrofitBucket();
    ...
    public static String getUrlPushProxy() {
        return NextcloudTalkApplication.Companion.getSharedApplication().getApplicationContext().getResources()
                .getString(R.string.nc_push_server_url) + "/devices";
    }
